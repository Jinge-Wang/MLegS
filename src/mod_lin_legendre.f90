...
! [NOTES]:
! CALCULATES *** 1/(N^M_N) *** WHERE N^M_N FOLLOWS THE DEFINITION OF 
! EQUATION (13) IN MATSUSHIMA & MARCUS (1997):
...
!=======================================================================
    ...
    INTEGER :: WARNING_SWITCH                                             ! PRINT OUT WARNING
    ! WARNING_SWITCH = 0: NO WARNING
    !                = 1: DISPLAY WARNING ONCE
    ...
    DO MM = 1, ME
      ...
      DO NN = 1, NE
        DO XX = 1, SIZE(X)
          ...
          ! When an entry of FM_LEG_TBL3 is smaller than the machine DP limit,
          ! a waning message will be given by default and the entry will be set to 0.
          ! ( FM library: TO_DP -> FM_2DP -> FMM2DP -> FMMD: line 17508 )
          ! To turn off the warning message, go to fmsave.f90: line 231 and set KWARN = 0.
          IF ((WARNING_SWITCH .GT. 0).AND.(LEG_TBL3(XX, NN, MM).EQ.0.D0)) THEN
            ...
            ELSE
              WRITE(6,"(' WARNING of LEG_TBL3: ( XX = ',I3,"  //  &
                      "', NN =  ', I3, ', MM = ', I3, ') ENTRY IS SMALLER THAN DP LIMIT - STORED AS ZERO')"    &
              ) XX, NN, MM
            ENDIF
