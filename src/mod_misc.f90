...
INTEGER , PARAMETER, PUBLIC :: P8 = SELECTED_REAL_KIND(P=15,R=307) ! DOUBLE PRECISION
...
61   FORMAT(4X,8I23)
60   FORMAT(I3,':',1P,8(S,E11.3E3,SP,E11.3E3,'i'))
...
!=======================================================================
SUBROUTINE LABOPEN(FN, NC, OVERWRITE)
!=======================================================================
! [USAGE]: 
! OPEN A FILE FN FOR READING OR WRITING.
! [PARAMETERS]:
! FN        >> FILENAME TO BE OPENED.
! NC        >> # OF VALUES EXPECTED PER LINE (FOR RECL CALCULATION).
! OVERWRITE >> (OPTIONAL) LOGICAL FLAG.
!              .TRUE. (DEFAULT): OVERWRITE FILE IF IT EXISTS.
!              .FALSE.: DO NOT OPEN (AND REPORT ERROR) IF FILE EXISTS.
! [DEPENDENCIES]:
! 1. READCHAR(~) @ MOD_MISC
! [UPDATES]:
! RE-CODED BY SANGJOON LEE @ NOV 10 2020
! MODIFIED BY GEMINI CODE ASSIST TO ADD OVERWRITE PROTECTION
!=======================================================================
IMPLICIT NONE
CHARACTER(LEN=*), INTENT(IN) :: FN
...
INTEGER :: IDMY, RECLEN, IS
CHARACTER(LEN=1) :: DMY
LOGICAL :: FILE_EXISTS, ALLOW_OVERWRITE

ALLOW_OVERWRITE = .TRUE.
IF (PRESENT(OVERWRITE)) ALLOW_OVERWRITE = OVERWRITE

INQUIRE(FILE=FN, EXIST=FILE_EXISTS)

IF (FILE_EXISTS .AND. .NOT. ALLOW_OVERWRITE) THEN
    WRITE(*,*) 'LABOPEN: FILE EXISTS AND OVERWRITE IS FALSE. CANNOT OPEN: ', TRIM(FN)
    STOP 'Execution stopped to prevent file overwrite.'
END IF

RECLEN = 25 * (NC + 1)
OPEN(UNIT=11, FILE=FN, STATUS='REPLACE', IOSTAT=IS, RECL=RECLEN)

IF (IS.NE.0) THEN
    WRITE(*,*) 'LABOPEN: CANNOT OPEN', TRIM(FN)
