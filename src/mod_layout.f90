...
!=======================================================================
      ...
      ! CREATE PSI AND CHI FOR Q-VORTEX PROFILE
      PUBLIC:: QVORTEX
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,NN,FAC,X,Y)
      DO MM=1,MIN(NTCHOP-A%INTH,SIZE(A%E,2)) !NTCHOP
        DO NN=1,NRCHOPS(MM+A%INTH) ! FFF HAS ALL RADIAL POINTS
        N = M(MM+A%INTH)+(NN-1)
          DO KK=1,SIZE(A%E,3) !NXCHOPDIM
...
!=======================================================================
      ...
      ! PPP: NDIMR/N1, NDIMTH, NDIMX/N2
      NRTMP = MIN(NR-A%INR,SIZE(A%E,1))  
      DO NN = 1,NRTMP !MIN(NR-A%INR,SIZE(A%E,1))
            IF (TFM%R(NN+A%INR).GT.ELL) THEN
            NRTMP = NN
            EXIT
            ENDIF
...
!$OMP END PARALLEL DO
     ...
     !CALL MSAVE(B%E(:,:,1),'firstPlanes.dat') ! MEMO>>>NEED TO CHANGE MSAVE FIRST
      ...
      END SUBROUTINE CHECKGAUSSIAN
!=======================================================================
      SUBROUTINE QVORTEX(PSI, CHI)
!=======================================================================
! [USAGE]: 
! CREATE PSI AND CHI COMPONENTS FOR THE Q-VORTEX PROFILE SPECIFIED IN QPAIR
! [PARAMETERS]:
! PSI (OUT): TYPE(SCALAR), POLOIDAL STREAM FUNCTION COMPONENT
! CHI (OUT): TYPE(SCALAR), TOROIDAL STREAM FUNCTION COMPONENT
!=======================================================================
  IMPLICIT NONE
  
  ! ARGUMENTS
  TYPE(SCALAR), INTENT(OUT) :: PSI, CHI
  ...
  IF (MPI_RANK == 0) THEN
    ...
    WRITE(*,*) 'CREATING Q-VORTEX PROFILE WITH', QPAIR%N, 'VORTICES'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  DO I = 1, QPAIR%N
    ...
    ! TO OBTAIN PSI0, WE USE: OMEGA_Z = -DELSQH PSI0 --> PSI0 = DELSQH^(-1)(-OMEGA_Z)
    GAUSDATA%XC = QPAIR%X(I)
    GAUSDATA%YC = QPAIR%Y(I)
    GAUSDATA%Q  = QPAIR%Q(I) * 2.0_P8
    GAUSDATA%B  = 1.0_P8
    ...
    ! CREATE GAUSSIAN VORTEX MULTIPLIED BY (1-X)^(-2)
    CALL LAY(A, GAUS, 2)
  ENDDO

  ! TRANSFORM (1-X)^(-2)*OMEGA_Z FROM PPP TO FFF SPACE
  CALL TOFF(A)
  
  ! NEGATE AND MUTE LOG TERM
  A%E = -A%E
  A%LN = 0.0_P8
  
  ! ADD RANDOM NOISE IF REQUESTED
  IF (QPAIR%NOISE == 1) THEN
    CALL ALLOCATE(B)
    B = A
  ...
  ! APPLY INVERSE HORIZONTAL LAPLACIAN: -(1-X)^(-2)*OMEGA_Z --> PSI0
  CALL ALLOCATE(B)
  CALL IDELSQH(A, B)
  
  ! OUTPUT PSI COMPONENT
  CALL ALLOCATE(PSI)
  PSI = B
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  DO I = 1, QPAIR%N
    ...
    ! TO OBTAIN CHI0, WE USE: V_Z = -DELSQH CHI0 --> CHI0 = DELSQH^(-1)(-V_Z)
    GAUSDATA%XC = QPAIR%X(I)
    GAUSDATA%YC = QPAIR%Y(I)
  ...
  ! TRANSFORM (1-X)^(-2)*V_Z FROM PPP TO FFF SPACE
  CALL TOFF(A)
  ...
  ! APPLY INVERSE HORIZONTAL LAPLACIAN: -(1-X)^(-2)*V_Z --> CHI0
  CALL ALLOCATE(B)
  CALL IDELSQH(A, B)
  
  ! OUTPUT CHI COMPONENT
  CALL ALLOCATE(CHI)
  CHI = B
  ...
  IF (MPI_RANK == 0) THEN
    WRITE(*,*) 'Q-VORTEX PROFILE CREATION COMPLETED'
  ENDIF

  RETURN
END SUBROUTINE QVORTEX 
!=======================================================================
!============================= FUNCTIONS ===============================
!=======================================================================
      FUNCTION RAN0(IDUM)
