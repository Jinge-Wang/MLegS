...
IF(PRESENT(SP) .AND. (SP .EQV. .TRUE.)) B = B / ELL2
...
CALL RTRAN_MK( DP_B, 1) ! DP_B = BU (PFF)

! PFF SPACE
ALLOCATE(DZ_B(NR),D_B(NR))
DZ_B(:NR) = DP_B(:NR)*IU*KV ! DZ_B = BU (PFF) * IU * K_0
DP_B(:NR) = DP_B(:NR)*IU*MV ! DP_B = BU (PFF) * IU * M_0

!$OMP PARALLEL DO DEFAULT(SHARED) SCHEDULE(static) &
!$OMP& PRIVATE(NN)
DO NN=1,NR
    D_B(NN) = ( RUR0(NN)*RDR_B(NN) + RUP0(NN)*DP_B(NN) )/TFM%R(NN)**2.D0 + UZ0(NN)*DZ_B(NN)
ENDDO
!$OMP END PARALLEL DO
...
NN=NRCHOPS(2)
BD(:NN) = D_B(:NN)
IF (NN < NSIZE) BD(NN+1:NSIZE) = 0.D0
...
COMPLEX(P8),DIMENSION(NSIZE),OPTIONAL,INTENT(IN):: BU
COMPLEX(P8),DIMENSION(NSIZE),OPTIONAL,INTENT(INOUT):: BD
...
IF ( PRESENT(EL).and.PRESENT(ER) ) THEN ! BOTH ER&EL, EV
    ...
    !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(I,ANG,PHASE_FACTOR)
    DO I = 1, NI
...
IF ((M(2).NE.M_ACTUAL).OR.(ABS(ZLEN-2*PI/AK_ACTUAL).GE.1.0E-13)) THEN
    WRITE(*,*) 'EIG2VEL: SET NEW M AND AK'
    ZLEN = 2*PI/AK_ACTUAL
    NTH = 2
    NX = 4
    NTCHOP = 2 ! M = 0, M = M_ACTUAL
...
SUBROUTINE SAVE_PSICHI(PSICHI,SAVE_FILENAME)
! ======================================================================
