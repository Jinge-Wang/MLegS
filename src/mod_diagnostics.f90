...
! ======================================================================
    ...
    IF (.NOT.ALLOCATED(MONITORDATA%MK)) RETURN
    ...
    C0 = CHI0
    C1 = CHI1
    P0%LN=0._P8; CALL RTRAN(P0,1) ! PFF SPACE
    P1%LN=0._P8; CALL RTRAN(P1,1)
    C0%LN=0._P8; CALL RTRAN(C0,1)
    C1%LN=0._P8; CALL RTRAN(C1,1)
    ...
    TEMP_TBL = 0.D0
    DO I=1,NUM
       IF (MONITORDATA%MK(I,1)<0) MONITORDATA%MK(I,:) = -MONITORDATA%MK(I,:)
       MM = MONITORDATA%MK(I,1)+1 ! INDEX
       KK = MONITORDATA%MK(I,2)+1 ! INDEX
       ...
       IF ((PSI1%INTH.LT.MM).AND.(PSI1%INTH+SIZE(PSI1%E,2).GE.MM)) THEN
         IF ((PSI1%INX.LT.KK).AND.(PSI1%INX+SIZE(PSI1%E,3).GE.KK)) THEN
            ...
            ! OBTAIN EIGENVALUES BASED ON THE LOWEST RADIAL MODES
    ...
    IF (MPI_RANK.EQ.0) THEN
      DO I = 1,NUM
         PRINT *,'MODE:',MONITORDATA%MK(I,1),MONITORDATA%MK(I,2)
            PRINT *,'EIGENVALUES FOR FIRST 8 RADIAL COLLOCATION POINTS'
    ...
    IF (MPI_RANK.EQ.0) THEN
      ...
      W = TFM%W
      DO I=1,NUM
         MM = MONITORDATA%MK(I,1)+1
         KK = MONITORDATA%MK(I,2)+1
   ...
   700    FORMAT(3E23.15)
         ...
         CLOSE(2000)
         !--
         PRINT *, 'FOR THE MODE(M,K)   = ', MONITORDATA%MK(I,1), MONITORDATA%MK(I,2)
...
SUBROUTINE PRINT_ENERGY_SPECTRUM(PSI,CHI,TAG,ESPEC_IN)
! ======================================================================
    IMPLICIT NONE
    TYPE(SCALAR), INTENT(IN):: PSI, CHI
    INTEGER::TAG
    REAL(P8), ALLOCATABLE, DIMENSION(:,:):: ESPEC
    REAL(P8), DIMENSION(:,:), OPTIONAL:: ESPEC_IN
    INTEGER::MM,KK,FILESTATUS
    REAL(P8)::TIME
    
    IF (PRESENT(ESPEC_IN)) THEN

       IF (MPI_RANK.EQ.0) THEN
         OPEN(UNIT=666,FILE=TRIM(ADJUSTL(FILES%SAVEDIR))//'especData_M.dat',STATUS='UNKNOWN',&
         & IOSTAT=FILESTATUS, POSITION='APPEND')
         TIME = TIM%T
         WRITE(666, '(*(E24.16,1X))') TIME, (SUM(ESPEC_IN(MM,:)),MM=1,NTCHOP)
         CLOSE(666)

         OPEN(UNIT=667,FILE=TRIM(ADJUSTL(FILES%SAVEDIR))//'especData_K.dat',STATUS='UNKNOWN',&
         & IOSTAT=FILESTATUS, POSITION='APPEND')
         TIME = TIM%T
         WRITE(667, '(*(E24.16,1X))') TIME, (SUM(ESPEC_IN(:,KK)),KK=1,NXCHOPDIM)
         CLOSE(667)
      ENDIF

      RETURN

    ELSE IF (TAG.EQ.0) THEN   !PRINT SPECTRUM FOR POSITIVE M'S AND K'S ONLY
    ...
    ELSE IF (TAG.EQ.1) THEN   !PRINT SPECTRUM FOR ALL K'S BUT POSITIVE M'S ONLY
    ...
    IF (MPI_RANK.EQ.0) THEN
      ...
      WRITE(666, '(*(E24.16,1X))') TIME, (SUM(ESPEC(MM,:)),MM=1,SIZE(ESPEC,1))         
      CLOSE(666)     
      ...
      WRITE(667, '(*(E24.16,1X))') TIME, (SUM(ESPEC(:,KK)),KK=1,SIZE(ESPEC,2))            
      CLOSE(667)  
    ENDIF
...
SUBROUTINE WRITE_ENERGY_DATA_TO_FILE(PSI,CHI,ESPEC_IN)
! ======================================================================
   IMPLICIT NONE
   TYPE(SCALAR):: PSI, CHI
   REAL(P8), ALLOCATABLE, DIMENSION(:,:):: ESPEC
   ...
   A = SIZE(MONITORDATA%MK,1)
   B = SIZE(MONITORDATA%MK,2)

   ALLOCATE( TEMP(A,B) )
   TEMP = MONITORDATA%MK

   ALLOCATE(ESPEC(NTCHOP,NXCHOPDIM))
   IF (PRESENT(ESPEC_IN)) THEN
      IF (SIZE(ESPEC_IN,1).NE.NTCHOP.OR.SIZE(ESPEC_IN,2).NE.NXCHOPDIM) THEN
         CALL MPRINT('ERROR: ESPEC_IN SIZE MISMATCH!')
         CALL MPI_ABORT(MPI_COMM_IVP, ERR_FLAGS%DIMMISMATCH, IERR)
      ENDIF
      ESPEC = ESPEC_IN
   ELSE
      ESPEC  = ENERGY_SPEC_MODIFIED(PSI,CHI)
...
! ======================================================================
! [USAGE]:
! COMPUTE ENERGY SPECTRUM OVER WAVENUMBER MAGNITUDES: E(|M|,|K|).
! [PARAMETERS]:
! PSI >> TOROIDAL COMPONENT IN PFF_SPACE.
! CHI >> POLOIDAL COMPONENT IN PFF_SPACE.
! [NOTES]:
! THIS FUNCTION COMPUTES THE ENERGY SPECTRUM OVER NON-NEGATIVE M AND K 
! MODES, SO THE DIRECTIONALITY OF THE MODES IS NOT CONSIDERED.
! CODED BY J. BARRANCO, 01/22/1999
! ======================================================================
   IMPLICIT NONE 
...
! ======================================================================
! [USAGE]:
! COMPUTE ENERGY SPECTRUM OVER ALL WAVENUMBERS: E(M,K).
! [PARAMETERS]:
! PSI >> TOROIDAL COMPONENT IN PFF_SPACE.
! CHI >> POLOIDAL COMPONENT IN PFF_SPACE.
! [NOTES]:
! THIS FUNCTION COMPUTES THE ENERGY SPECTRUM OVER ALL M AND K MODES.
! FOR REAL FIELDS IN PHYSICAL SPACE, HERMITIAN SYMMETRY REQUIRES ONLY
! NON-NEGATIVE M (BUT ALL K'S) TO BE STORED.
! CODED BY J. WANG
! ======================================================================
    IMPLICIT NONE
    TYPE(SCALAR):: PSI,CHI,W1,W2
   ...
   !  TYPE(SCALAR):: RUR,RUP,UZ,RUR2,RUP2,UZ2,UP,RUPXX
    ...
    CALL RTRAN(W1,1)
    CALL RTRAN(W2,1)
    ENERGY_SPEC_MODIFIED = -PRODCT_MK(W1,W2)
    ...
    CALL RTRAN(W1,1)
    CALL RTRAN(W2,1)
    ENERGY_SPEC_MODIFIED = ENERGY_SPEC_MODIFIED + PRODCT_MK(W1,W2)
    CALL CHOPSET(-3)
    CALL DEALLOCATE(W1)
    CALL DEALLOCATE(W2)
...
! 1. MAXVELP(~) @ MOD_MARCH
! [NOTE]:
! MONITORDATA%MK MUST BE CONSISTENT WITH THE PERTURBATION ADDED.
! MONITORDATA%MK{1,:} -> M_BAR, K_BAR
! MONITORDATA%MK{2,:} -> M_1, K_1
! MONITORDATA%MK{3,:} -> M_2, K_2
! [UPDATES]:
...
! ======================================================================
   ...
   ! RENORMALIZE
   MON_SIZE = SIZE(MONITORDATA%MK,1)
   DO MON_II = 1,MIN(MON_SIZE,3)
      ...
      !       IT TO {M,-K}. ALSO, THE LINE CHANGES MONITORDATA%MK, WHICH IS A GLOBAL VARIABLE -> THIS IS BAD.
      !       CURRENTLY, ALL 3 ADDED MODES ARE POSITIVE, SO THIS IS FINE FOR NOW.
      IF (MONITORDATA%MK(MON_II,1)<0) MONITORDATA%MK(MON_II,:) = -MONITORDATA%MK(MON_II,:)
      MM = MONITORDATA%MK(MON_II,1)+1 ! INDEX
      KK = MONITORDATA%MK(MON_II,2)+1 ! INDEX
      IF (KK<1) KK=KK+2*NXCHOP-1
...
! ======================================================================
   ...
   IF (MONITORDATA%MK(2,1)<0) MONITORDATA%MK(2,:) = -MONITORDATA%MK(2,:)
   MM = MONITORDATA%MK(2,1)+1 ! INDEX
   KK = MONITORDATA%MK(2,2)+1 ! INDEX
   IF (KK<1) KK=KK+2*NXCHOP-1
