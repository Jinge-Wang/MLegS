...
*   **Scheme 2: Exponential Time Differencing (ETD2)** (when `BSNSQ%ADAMS = .FALSE.`)
    ...
    *   **Linear Wave Terms**: The fast linear wave dynamics due to rotation (inertial waves) and stratification (internal gravity waves) are solved exactly using an exponential integrator. The core of this method is in `CALC_BOUSS_DIAG`, which diagonalizes the linear operator `L`, and `CALC_BOUSSI_ETD_OP`, which pre-computes the exponential and related matrices.
...
*   **Available Potential Energy Calculation**: In `CALC_BOUSSI_ENERGY`, the potential energy calculated is the available potential energy (APE) defined as `∫ B² dV / 2N²`.
...
*   **Legacy Code**: The `mod_march` module contains some older, likely unused time-stepping subroutines (`STEP_INCOMP_AB_CN`, `RICH_INCOMP_FE_BE`, `EULER`) alongside the newer `SOLVER_T` implementation. Refactoring to remove this legacy code would improve maintainability.
