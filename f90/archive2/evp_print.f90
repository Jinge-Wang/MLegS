...
! DO M_BAR = -1,1,2
    ...
    WRITE(*,*) MPI_GLB_RANK,':',M_BAR,K_BAR
        ...
        IF (MPI_GLB_RANK .EQ. JJ) THEN
            ...
            DO II = 1,SIZE(M_eig)
                ...
                ! IF ((EIGRES(EIG_R_BAR)).OR.(II.LE.4)) THEN
                    ...
                    call SAVE_VEL(RUR_BAR, RUP_BAR, UZ_BAR, './converg/check3/vel_MK_'//ITOA3(M_BAR)//'_'//K_VAL &
                                //'_IND_'//ITOA3(II)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
                    ! call SAVE_VEC('./converg/check3/PC_o_MK_'//ITOA3(M_BAR)//'_'//K_VAL &
                    !             //'_IND_'//ITOA3(II)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output',EIG_R_BAR)
                    DEALLOCATE(RUR_BAR, RUP_BAR, UZ_BAR)
                ENDIF
            ENDDO
            DEALLOCATE(EIG_R_BAR,M_eig)
            ! WRITE(*,*) EIGRES(EIG_R_mat,M_BAR)
