PROGRAM EVP_ALMOST_DISCRETE_0909
!=======================================================================
! [USAGE]:
...
CALL EIG_MATRIX(M_BAR, K_BAR, M_mat, M_eig, EIG_R_mat, EIG_L_mat, comm_grp=newcomm, print_switch=.true.) ! calculate operator matrix and its corresponding eigenvalues
...
DO II = 1,6
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        ! WRITE(*,*) tol_diff, EIG_BAR
        ! WRITE(*,*) MASK_1
        ! WRITE(*,*) MASK_2
        ! DO JJ = 1,SIZE(MASK_1); DO KK = 1,SIZE(MASK_2)
        !     IF (MASK_1(JJ).AND.MASK_2(KK)) WRITE(*,*) JJ,KK,ABS(AIMAG(MESHGRID(M_eig_1_0, M_eig_2_0) - EIG_BAR))
        ! END DO; END DO
...
IF (MPI_GLB_RANK.EQ.0) THEN
    ...
    ! ! ANALYSIS: SAVE CROSS PRODUCT
    ! ! BAR X R1
    ! WRITE(K_VAL,'(F06.2)') K_VAL_2_1
    ! call SAVE_VEL(ROR_1,ROP_1,OZ_1, './converg/cross_MK_'//ITOA3(M_2)//'_'//K_VAL &
    !             //'_IND_'//ITOA3(KK)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
    ! ! BAR X R1
    ! WRITE(K_VAL,'(F06.2)') K_VAL_1_1
    ! call SAVE_VEL(ROR_2,ROP_2,OZ_2, './converg/cross_MK_'//ITOA3(M_1)//'_'//K_VAL &
    !             //'_IND_'//ITOA3(KK)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
...
END PROGRAM EVP_ALMOST_DISCRETE_0909
!=======================================================================
