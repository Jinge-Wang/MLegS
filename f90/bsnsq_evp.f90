...
INTEGER:: FID, STAT, II_SEL
CHARACTER(LEN=256) :: DIR_PATH, INPUT_BUF
LOGICAL :: DO_SAVE_MODE
...
! Usage: mpirun -n N ./bin/bsnsq_evp_exec -m 1 -k 10.0 [--save-mode]
! ======================================================================
M_BAR = 1; K_BAR = 10.0_P8   ! defaults
...
IF (MPI_GLB_RANK == 0) THEN
    ...
    DO WHILE (I_ARG <= NUM_ARGS)
        ...
        I_ARG = I_ARG + 1
    END DO
    WRITE(*,'(A,I0,A,F0.4)') 'Running EVP | M = ', M_BAR, ' | K = ', K_BAR
END IF
CALL MPI_BCAST(M_BAR, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, IERR)
CALL MPI_BCAST(K_BAR, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERR)
...
CALL EIG_MATRIX_BSNSQ2(M_BAR, K_BAR, BSNSQ_H, BSNSQ_V, M_mat, M_eig, EIG_VEC_R = EIG_R_mat, EIG_VEC_L = EIG_L_mat, comm_grp=newcomm, serial_switch=.false.) 
NR_MK = NRCHOPS(2)
...
ELSE
    ...
    ! ----------------------------------------------------------------
    ! OPTIONAL: Save a selected eigenvector as perturb.input.
    ! Activated by passing '--save-mode' (or '-s') as a command-line
    ! argument (can appear alongside -m / -k flags).
    ! ----------------------------------------------------------------
    DO_SAVE_MODE = .FALSE.
    NUM_ARGS = COMMAND_ARGUMENT_COUNT()
    DO I_ARG = 1, NUM_ARGS
        CALL GET_COMMAND_ARGUMENT(I_ARG, ARG_STR)
        IF (TRIM(ARG_STR) == '--save-mode' .OR. TRIM(ARG_STR) == '-s') THEN
            DO_SAVE_MODE = .TRUE.
            EXIT
        END IF
    END DO

    IF (DO_SAVE_MODE) THEN
        WRITE(*,*)
        WRITE(*,'(2A)') '  Eigenvalue list saved to: ', TRIM(FILENAME)
        WRITE(*,'(A,I0,A)') '  Indices run from 1 to ', SIZE(M_eig), '.'
        WRITE(*,'(A)') '  Enter mode index to save as ./data/perturb.input'
        WRITE(*,'(A)') '  (press Enter without a number to abort):'
        READ(*, '(A)', IOSTAT=STAT) INPUT_BUF
        IF (STAT == 0 .AND. LEN_TRIM(INPUT_BUF) > 0) THEN
            READ(INPUT_BUF, *, IOSTAT=STAT) II_SEL
            IF (STAT == 0 .AND. II_SEL >= 1 .AND. II_SEL <= SIZE(M_eig)) THEN
                ! The EVP eigenvector is stored as [PSI, DEL2CHI, B].
                ! bsnsq_mode needs [PSI, CHI, B], so invert the Laplacian
                ! on the DEL2CHI block before writing the file.
                ! LEGINIT is still configured for (M_BAR, K_BAR) here.
                ALLOCATE(EIG_R_BAR(3*NR_MK), DEL2CHI_BAR(NRCHOPDIM), CHI_BAR(NRCHOPDIM))
                EIG_R_BAR = EIG_R_mat(:3*NR_MK, II_SEL)
                DEL2CHI_BAR = 0.D0
                DEL2CHI_BAR(:NR_MK) = EIG_R_BAR(NR_MK+1:2*NR_MK)
                CALL IDEL2_MK(DEL2CHI_BAR, CHI_BAR)
                EIG_R_BAR(NR_MK+1:2*NR_MK) = CHI_BAR(:NR_MK)
                CALL SAVE_PERTURB_BSNSQ('./data/perturb.input', M_BAR, K_BAR, &
                    EIG_R_BAR, M_eig(II_SEL))
                WRITE(*,'(A,I0,A)') '  Mode index ', II_SEL, &
                    ' saved to: ./data/perturb.input'
                DEALLOCATE(EIG_R_BAR, DEL2CHI_BAR, CHI_BAR)
            ELSE
                WRITE(*,'(A)') '  Index out of range or invalid. No mode profile saved.'
            END IF
        ELSE
            WRITE(*,'(A)') '  No index provided. No mode profile saved.'
        END IF
    END IF
...
!=======================================================================
! [USAGE]: 
! CONVERT TO PRIMITIVE VARIABLES, APPLY THE OPERATOR, THEN PROJECT BACK
!=======================================================================
IMPLICIT NONE
...
! Must be ALLOCATABLE because RTRAN_MK dynamically resizes them
COMPLEX(P8),DIMENSION(:),ALLOCATABLE:: RURU, RUPU, UZU, BU_COPY

! Temporary scalars to prevent memory overwrites during matrix mult
...
! --- Zero-Pad Unresolved Modes ---
! Using SIZE() check and unbounded slicing (NR+1:) because RTRAN_MK 
! may have altered the total size of the arrays from NSIZE.
