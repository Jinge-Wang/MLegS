...
! INTEGER:: SIZE1, SIZE2, SIZE3, ARRAYSIZE, ELEMENT_SIZE, FILEBLK, LOCAL_ARRAYSIZE
! INTEGER(KIND=MPI_ADDRESS_KIND):: DISPLACEMENT

! ! INPUT:
! COMPLEX(P8),DIMENSION(:,:,:),ALLOCATABLE:: LOCAL_ARRAYS
! ! OUTPUT:
! integer,DIMENSION(:,:,:),ALLOCATABLE:: GLOBAL_ARRAYS
...
! CREATE LOCAL SCALAR
call ALLOCATE(local_scalar, PPP_SPACE)
...
DO K = 1,SIZE(local_scalar%E,3)
    DO J = 1,SIZE(local_scalar%E,2)
        DO I = 1,SIZE(local_scalar%E,1)
            local_scalar%E(I,J,K) = (I+local_scalar%INR)*10000 & 
                                  + (J+local_scalar%INTH)*100 &
                                  + (K+local_scalar%INX)
