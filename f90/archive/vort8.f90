...
do it=1,iii
   ...
   ENDIF
   if ((abs(eig1-eig10)/abs(eig10).lt.1e-8) .AND. (abs(eig2-eig20)/abs(eig20).lt.1e-8) .AND. (tim%t.ge.1110)) GOTO 999
...
!    ENDIF
!    ! M = 0, K != 0
!    IF (((MONITORDATA%MK(2,1).EQ.0).AND.(II+PSI%INTH .EQ. MONITORDATA%MK(2,1)+1).AND.(JJ+PSI%INX .EQ. 2*NXCHOP-MONITORDATA%MK(2,2))) &
!       .OR. ((MONITORDATA%MK(3,1).EQ.0).AND.(II+PSI%INTH .EQ. MONITORDATA%MK(3,1)+1).AND.(JJ+PSI%INX .EQ. 2*NXCHOP-MONITORDATA%MK(3,2)))) THEN
!       psi%e(:,II,JJ) = psi%e(:,II,JJ) + dpsi%e(:,II,JJ)*TIM%DT
!       chi%e(:,II,JJ) = chi%e(:,II,JJ) + dchi%e(:,II,JJ)*TIM%DT
!    ENDIF
! ENDDO; ENDDO
