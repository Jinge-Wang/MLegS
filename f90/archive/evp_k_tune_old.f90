...
105  FORMAT('         -(', I3,' - INPUT) (INPUT  >', I3, ')')
    ...
    K_VAL0 = 8.51 ! A random starting guess
    tol = 1.0E-3 ! 0.1%
    K_delta = 0.0001
    ...
    ! Find possible K_shift
        ...
        DO II = 1,2*N_mat
            K_tune(II) = (AIMAG(EIG_TARGET) - AIMAG(M_eig0(II)))/AIMAG(DOT_PRODUCT(EIG_L_mat0(:,II),EIG_R_mat1(:,II)))

            IF (K_tune(II).GT.1000.D0) THEN 
                K_tune(II) = 1000.D0
            ELSEIF (K_tune(II).LT.-1000.D0) THEN
                K_tune(II) = -1000.D0
            ENDIF
            K_tune(II) = K_tune(II)*K_delta
            IF ((K_tune(II)+K_VAL0).LT.0.D0) THEN
