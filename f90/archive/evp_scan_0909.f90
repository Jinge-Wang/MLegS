...
DO M_BAR = 0,10; DO K_BAR_SCAN = -10.D0,10.D0,1.D0
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        open(unit=LOGID,FILE='discrete_scan.log',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
        WRITE(LOGID,*) 'M_BAR,KBAR: ',ITOA3(M_BAR),',',ITOA3(K_BAR_SCAN)
        close(LOGID)
    ENDIF
...
DO M_1 = -M_BAR/2,(10-M_BAR); DO K_1_SCAN = -10.D0,10.D0,1.D0
    ...
    IF (MPI_GLB_RANK.EQ.0) WRITE(*,*) SUBCOMM_1,SUBCOMM_2
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        open(unit=LOGID,FILE='discrete_scan.log',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
        WRITE(LOGID,*) 'M_1,K_1: ', ITOA3(M_1),',',ITOA3(K_1_SCAN)
        close(LOGID)
    ENDIF
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        IF ((tol_diff.LT.tol).OR.(ABS(K_shift).LE.1.0E-4)) THEN
            ...
            WRITE(LOGID,*) 'Desired tol reached w/ ', ITOA3(II-1),' steps (tol_diff = ', tol_diff,')'
            WRITE(LOGID,*) 'DEGEN:'
