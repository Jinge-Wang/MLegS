PROGRAM EVP_ALMOST_DISCRETE_SCAN2_COPY
!=======================================================================
! [USAGE]: 
! Faster Version: the bar term is calculated once for each wavenumber
! This version is for stampede2
! 
...
!=======================================================================
    ...
    INTEGER     :: M_BAR,M_1,M_2,EIG_BAR_IND,M_BAR_LIST(3),M_BAR_II
    ...
    ! SCANNING:
    CHARACTER(LEN=72):: FILENAME,FILEPATH
    INTEGER:: FID, K_BAR_SCAN, K_1_SCAN, JUMP_FLAG
    ...
    FILEPATH = 'FOLDER_scratch/SearchCombined_node1/14'
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        WRITE(*,*) 'PROGRAM 14 STARTED'
        CALL PRINT_REAL_TIME()   ! @ MOD_MISC
        open(unit=52,FILE=TRIM(ADJUSTL(FILEPATH))//'/discrete_scan.log',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
        ! open(unit=52,FILE='discrete_scan.log',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
    ENDIF
...
M_BAR_LIST = (/1,5,9/)    
DO M_BAR_II = 1,3
M_BAR = M_BAR_LIST(M_BAR_II)
DO K_BAR_SCAN = 1.D0,4.D0,1.D0
! M_BAR_LIST = (/2,3,4/)    
! DO M_BAR_II = 1,2
! M_BAR = M_BAR_LIST(M_BAR_II)
! DO K_BAR_SCAN = 2.D0,2.D0,1.D0
K_BAR = REAL(K_BAR_SCAN)
    IF (MPI_GLB_RANK.EQ.0) write(52,*) M_BAR,K_BAR
...
JUMP_FLAG = 0   
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        write(52,*) 'TARGET EIG_BAR:', EIG_BAR
    ! =================== CHECK IF CRITICAL LAYER MODE =====================
        IF (M_BAR.LE.3) THEN
            IF (IMAG(EIG_BAR).GT.-(M_BAR*QPAIR%Q(1) + K_BAR*QPAIR%H(1))) JUMP_FLAG = 39 !CALL MPI_ABORT(MPI_COMM_WORLD,1,IERR)
        ELSE
            IF (IMAG(EIG_BAR).GT.-(M_BAR*QPAIR%Q(1) + K_BAR*QPAIR%H(1))*1.005) JUMP_FLAG = 39!CALL MPI_ABORT(MPI_COMM_WORLD,1,IERR)
        ENDIF
        
    ENDIF
    CALL MPI_BCAST(JUMP_FLAG,1,MPI_INT,0,MPI_COMM_WORLD,IERR)
    IF (JUMP_FLAG.EQ.39) THEN
        IF (MPI_GLB_RANK.EQ.0) write(52,*) 'CYCLE IND'
        CYCLE loop_ind
    ENDIF
    ! ======================================================================
...
! !=======================================================================    
DO M_1 = 1,(10-M_BAR)
DO K_1_SCAN = 1.D0,10.D0,1.D0
! DO M_1 = 1,1
! DO K_1_SCAN = 8.D0,10.D0,1.D0
IF ((M_BAR.EQ.M_1).AND.(K_BAR_SCAN.EQ.K_1_SCAN)) CYCLE
CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
IF (MPI_GLB_RANK.EQ.0) write(52,*) M_BAR,K_BAR_SCAN,EIG_BAR_IND,M_1,K_1_SCAN
...
! !=======================================================================
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        ! write(52,*) M_BAR, K_BAR, EIG_BAR_IND, M_1, K_VAL_1_0
    ...
    ! Find possible K_shift
        ...
        write(52,*) 'K_shift:', K_shift
        ! write(52,*) 'Original diff:'
        ! write(52,*) MINVAL(ABS(AIMAG(M_eig_1_0-EIG_BAR))/ABS(AIMAG(EIG_BAR)))
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        IF ((tol_diff.LT.tol).OR.(ABS(K_shift).LE.1.0E-4)) THEN
            write(52,*) 'Desired tol reached'
            write(52,*) 'tol_diff:', tol_diff
            write(52,*) 'Number of steps:', II
            tol = 999.0
            ! GOTO 20
        ENDIF
        K_VAL_1_0 = K_VAL_1_1
...
CALL EIG_MATRIX(M_2,K_VAL_2_1,M_mat_2_1,M_eig_2_1,EIG_R_mat_2_1,EIG_L_mat_2_1,comm_grp=newcomm)
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        write(52,201) M_BAR, K_BAR, EIG_BAR_IND, EIG_BAR
        write(52,201) M_1, K_VAL_1_1, JJ, M_eig_1_1(JJ)
        write(52,201) M_2, K_VAL_2_1, kk, M_eig_2_1(KK)
...
write(52,*) 'LAMBDA RANGE: '
write(52,*) 'MIN: ', LAMB_LOW
write(52,*) 'MAX: ', LAMB_UP
...
! write(52,*) 'Customized Lambda value: '
! READ(5,'(F8.6)') LAMB_CUST
LAMB_CUST = 0.01
EIG_PRIME_R_custom = (0.25*(EIG_BAR+M_eig_1_1(JJ)-M_eig_2_1(KK))**2+LAMB_CUST**2*EIG_PRIME_R)**0.5
write(52,*) ''
write(52,*) "Lambda*SIGMA'(nearly degen)= +/- ",EIG_PRIME_R_L,'~',EIG_PRIME_R_U
write(52,*) "Lambda*SIGMA'(nearly degen)= +/- ",EIG_PRIME_R_custom,' (',LAMB_CUST,')'
write(52,*) " SIGMA(degen)'^2= ",EIG_PRIME_R
write(52,*) " SIGMA(degen)'  = ",EIG_PRIME_R**0.5
...
write(52,*) " BETA(degen)    = ",BETA

! ======================================================================
    IF (abs(REAL(EIG_PRIME_R**0.5)).GT.0.01) THEN
        open(unit=FID,FILE=TRIM(ADJUSTL(FILEPATH))//'/perturb_scan.GROW',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
        ! open(unit=FID,FILE='perturb_scan.GROW',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
        WRITE(FID,*) ''
        WRITE(FID,*) 'NEARLY DEGEN: *GROWTH*'
    ELSE
        open(unit=FID,FILE=TRIM(ADJUSTL(FILEPATH))//'/perturb_scan.NEGLIGIBLE',STATUS='unknown',ACTION='WRITE',ACCESS='APPEND',IOSTAT=IS)
...
10  IF(ALLOCATED(M_mat_1_0)) DEALLOCATE(M_mat_1_0)
    ...
    IF(ALLOCATED(VECR_B1)) DEALLOCATE(VECR_B1,VECR_B2,VECL_1,VECL_2)
...
ENDDO ! M_1
    ...
    IF(ALLOCATED(VECL_BAR)) DEALLOCATE(VECL_BAR)
ENDDO loop_ind! IND
    IF(ALLOCATED(M_mat)) DEALLOCATE(M_mat)
...
END PROGRAM EVP_ALMOST_DISCRETE_SCAN2_COPY
!=======================================================================
