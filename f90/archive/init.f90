...
! CALL MASSEMBLE(A%E,GLB_ARRAY,2)
! CALL save_glb(GLB_ARRAY,"A_GLB_PPP")
! DEALLOCATE(GLB_ARRAY)

! CALL HORFFT(A,-1)
! ALLOCATE(GLB_ARRAY(NDIMR, NDIMTH, NDIMX))
! CALL MASSEMBLE(A%E,GLB_ARRAY,2)
! CALL save_glb(GLB_ARRAY,"A_GLB_PFP")
! DEALLOCATE(GLB_ARRAY)

! CALL VERFFT(A,-1)
! ALLOCATE(GLB_ARRAY(NDIMR,NTCHOPDIM,NXCHOPDIM))
! CALL MASSEMBLE(A%E,GLB_ARRAY,1)
! CALL save_glb(GLB_ARRAY,"A_GLB_PFF")
! DEALLOCATE(GLB_ARRAY)
...
! CALL MASSEMBLE(A%E,GLB_ARRAY,1)
! CALL save_glb(GLB_ARRAY,"A_GLB_FFF")
! DEALLOCATE(GLB_ARRAY)
! ! ======================================================================
...
! ! ======================================================================
! ALLOCATE(GLB_ARRAY(NRCHOPDIM,NTCHOPDIM,NXCHOPDIM))
! CALL MASSEMBLE(B%E,GLB_ARRAY,1)
! CALL save_glb(GLB_ARRAY,"PSI0_GLB_FFF")
! DEALLOCATE(GLB_ARRAY)
! ! ======================================================================
...
! note: the original rtran gave slightly different results depending on
! the number of MPI processors. the differences come from matrix multipl
! -ication. therefore, i write this routine to replace matrix multiplica
! -tion with matrix-vector multiplication + loop. this might be slower,
! but the results are forced to be consistent. additionally, fortran is
! column-major, so the next routine rtran_test2 should be more efficient
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(NN,BE,BO,MM,IIII)
  DO MM=1, THSIZE !NTCHOP
    ...
    DO IIII=1,XSIZE
      ...
      BB%E(1:NN:2,MM,IIII:IIII)=TRANSPOSE(TFM%PF(:NRH,1:NN:2,MM+BB%INTH)) .MUL. BE(:,IIII:IIII)
      
      IF (NN.GE.2) THEN
      ! BB%E(2:NN:2,MM,:)=TRANSPOSE(TFM%PF(:NRH,2:NN:2,MM+AA%INTH)) .MUL. BO
      BB%E(2:NN:2,MM,IIII:IIII)=TRANSPOSE(TFM%PF(:NRH,2:NN:2,MM+BB%INTH)) .MUL. BO(:,IIII:IIII)
      ENDIF
    ENDDO

    ! IF ((MM+AA%INTH.EQ.1).and.(AA%INX.EQ.0)) THEN
    !   WRITE(*,*) NRH,NRCHOPS(1)
    !   WRITE(*,*) 'NN = ',NN,'size(PF(:NRH,2:NN:2,MM+BB%INTH)) - ', size(TFM%PF(:NRH,2:NN:2,MM+BB%INTH),1), size(TFM%PF(:NRH,2:NN:2,MM+BB%INTH),2),'size(BO(:,:)) -', size(BO(:,:),1), size(BO(:,:),2)
    !   WRITE(*,*) 'BB(:,1,1)'
    !   CALL MCAT(BB%E(:,1,1))
    ! ENDIF
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(NN,BE,BO,MM,IIII)
  DO MM=1, THSIZE !NTCHOP
    ...
    ! TFM%PF(RadialCollocPts, RadialModes, AzimuthalModes)
    ! BB%E(1:NN:2,MM,:)=TRANSPOSE(TFM%PF(:NRH,1:NN:2,MM+AA%INTH)) .MUL. BE
    BB%E(1:NN:2,MM,:)=TRANSPOSE(BE .MUL. TFM%PF(:NRH,1:NN:2,MM+BB%INTH))
    
    IF (NN.GE.2) THEN
    ! BB%E(2:NN:2,MM,:)=TRANSPOSE(TFM%PF(:NRH,2:NN:2,MM+AA%INTH)) .MUL. BO
    BB%E(2:NN:2,MM,:)=TRANSPOSE(BO .MUL. TFM%PF(:NRH,2:NN:2,MM+BB%INTH)) 
    ENDIF
    ...
    !   ! CALL MCAT(TRANSPOSE(BE(1:1,:) .MUL. TFM%PF(:NRH,1:NN:2,MM+BB%INTH)))
    !   ! WRITE(*,*) 'TRANSPOSE(TFM%PF(:NRH,NN,MM+BB%INTH)) .MUL. BO'
    !   ! CALL MCAT(TRANSPOSE(BO(:XSIZE-32,:) .MUL. TFM%PF(:NRH,2:NN:2,MM+BB%INTH)))
    !   ! CALL MCAT(MATMUL(TRANSPOSE(TFM%PF(:NRH,2:NN:2,MM+BB%INTH)),BO(:,:)))
    !   WRITE(*,*) 'BB(:,1,1)'
    !   CALL MCAT(BB%E(:,1,1))
    ! ENDIF
