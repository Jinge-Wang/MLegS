PROGRAM EVP_K_TUNE
    ...
    !=======================================================================
        ...
        COMPLEX(P8),DIMENSION(:,:),ALLOCATABLE:: M_mat, EIG_R_mat, M_mat_diff_1,M_mat_diff_2
        ...
        TYPE(SCALAR):: PSI_BAR,CHI_BAR,RUR_BAR,RUP_BAR,UZ_BAR,ROR_BAR,ROP_BAR,OZ_BAR
    ...
    105  FORMAT(' ***         -(', I3,' - INPUT) (INPUT  >', I3, ')')
        ...
        ! CALL MLOAD(TRIM(ADJUSTL(FILES%SAVEDIR))//FILES%PSI0,PSI_BAR)
        ! CALL MLOAD(TRIM(ADJUSTL(FILES%SAVEDIR))//FILES%CHI0,CHI_BAR)

        PSI_BAR%LN = 0.D0
        CHI_BAR%LN = 0.D0
        PSI_BAR%E = 0.D0
        CHI_BAR%E = 0.D0
        PSI_BAR%E(:NRCHOPS(M_IND_bar),M_IND_bar,K_IND_bar) = EIG_R_mat(:NRCHOPS(M_IND_bar),EIG_BAR_IND)
        CHI_BAR%E(:NRCHOPS(M_IND_bar),M_IND_bar,K_IND_bar) = EIG_R_mat(NRCHOPS(M_IND_bar)+1:,EIG_BAR_IND)
        CALL IDEL2(CHI_BAR,CHI_BAR)
        CALL RTRAN(PSI_BAR,1)
        CALL RTRAN(CHI_BAR,1) ! PFF
        PSI_BAR%E(:,1,1) = PSI_BAR%E(:,M_IND_bar,K_IND_bar)
        CHI_BAR%E(:,1,1) = CHI_BAR%E(:,M_IND_bar,K_IND_bar)
        PSI_BAR%E(:,M_IND_bar,K_IND_bar) = 0.D0
        CHI_BAR%E(:,M_IND_bar,K_IND_bar) = 0.D0

        CALL TOFF(PSI_BAR)
        CALL TOFF(CHI_BAR)       
        CALL MSAVE(PSI_BAR%E(:,1,1),'Psi_bar_FFF.dat' )
        ...
        CALL TOFF(PSI_BAR)
        CALL TOFF(CHI_BAR) ! FFF
        CALL MSAVE(PSI_BAR%E(:,1,1),'Psi_bar_FFF2.dat' )
    ...
    !     !     ALLOCATE(M_eig_1_1_ND(N_mat1*2))
    !     !     DO II = 1,2*N_mat1
    !     !         M_eig_1_1_ND(II) = M_eig_1_0(II) + K_shift*DOT_PRODUCT(EIG_L_mat_1_0(:,II),M_mat_diff_1(:,II))/DOT_PRODUCT(EIG_L_mat_1_0(:,II),EIG_R_mat_1_0(:,II)) ! DOT_PRODUCT(A,B) = SUM(CONJG(A)*B)
    !     !     ENDDO
    !     !     CALL MSAVE(M_eig_1_1_ND,'M_eig_1_1_ND.dat')
    ...
    !     DEALLOCATE(M_mat_2_1, M_mat_diff_2)
