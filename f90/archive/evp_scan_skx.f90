...
DO M_BAR = 0,10; DO K_BAR_SCAN = -10.D0,10.D0,1.D0
    ...
    IF (MPI_GLB_RANK.EQ.0) WRITE(LOGID,*) 'M_BAR,KBAR: ',ITOA3(M_BAR),',',ITOA3(K_BAR_SCAN)
...
loop_ind: DO EIG_BAR_IND = 1,size(M_mat,1)
    ...
    IF (JUMP_FLAG.EQ.39) THEN
        ! IF (MPI_GLB_RANK.EQ.0) WRITE(LOGID,*) 'CYCLE IND'
        CYCLE loop_ind
    ENDIF
    ! ======================================================================
...
DO M_1 = -M_BAR/2,(10-M_BAR); DO K_1_SCAN = -10.D0,10.D0,1.D0
    ...
    IF (MPI_GLB_RANK.EQ.0) WRITE(LOGID,*) 'M_1,K_1: ', ITOA3(M_1),',',ITOA3(K_1_SCAN)
    ...
    ! Find possible K_shift
        ...
        WRITE(LOGID,*) 'step#',ITOA3(II),'- K_shift:', K_shift
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        IF ((tol_diff.LT.tol).OR.(ABS(K_shift).LE.1.0E-4)) THEN
            WRITE(LOGID,*) 'Desired tol reached'
            WRITE(LOGID,*) 'tol_diff:', tol_diff, ', Number of steps:', II
            tol = 999.0
        ENDIF
        K_VAL_1_0 = K_VAL_1_1
    ENDIF
...
!=======================================================================
    IF (II.EQ.6) THEN ! Exceeds maximum number of steps allowed
        IF (MPI_GLB_RANK.EQ.0) THEN
            WRITE(LOGID,*) 'Seem trapped'
            WRITE(LOGID,*) 'tol_diff:', tol_diff
        ENDIF
...
20  CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        WRITE(LOGID,201) M_1, K_VAL_1_1, JJ, M_eig_1_1(JJ)
        WRITE(LOGID,201) M_2, K_VAL_2_1, kk, M_eig_2_1(KK)
