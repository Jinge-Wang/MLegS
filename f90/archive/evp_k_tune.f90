...
105  FORMAT('         -(', I3,' - INPUT) (INPUT  >', I3, ')')
    ...
    CALL ROOT_RANGE(K_VAL1,K_VAL0,20.D0,2.D0,M_IND2,EIG_TARGET)
    ! K_VAL0 = 8.43 ! lower bound
    K_delta = 0.001
    ...
    ! Find possible K_shift
        ...
        DO II = 1,2*N_mat
        K_tune(II) = K_delta*(  (AIMAG(EIG_TARGET) - AIMAG(M_eig0(II))) / AIMAG(DOT_PRODUCT(EIG_L_mat0(:,II),M_mat_diff(:,II))/DOT_PRODUCT(EIG_L_mat0(:,II),EIG_R_mat0(:,II)))  )
        ENDDO
        
        K_shift = K_tune(MINLOC(ABS(K_tune),1))
        !K_shift = MINVAL(ABS(K_tune))

        ! WRITE(*,*) 'K_shift:'
        ! WRITE(*,*) K_shift
        ! WRITE(*,*) 'Original diff:'
        ! WRITE(*,*) MINVAL(ABS(AIMAG(M_eig0-EIG_TARGET))/ABS(AIMAG(EIG_TARGET)))
    ...
    ! Match the IMAG part
        tol_diff = MINVAL(ABS(AIMAG(M_eig1-EIG_TARGET)))
    ...
    !     DO II = 1,2*N_mat
    !         M_eig1_ND(II) = M_eig0(II) + K_shift*DOT_PRODUCT(EIG_L_mat0(:,II),M_mat_diff(:,II))/DOT_PRODUCT(EIG_L_mat0(:,II),EIG_R_mat0(:,II)) ! DOT_PRODUCT(A,B) = SUM(CONJG(A)*B)
    !     ENDDO
