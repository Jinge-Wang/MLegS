PROGRAM EVP_K_TUNE_ROUGH
!=======================================================================
! [USAGE]: 
! USE VPROD_PFF RATHER THAN VPROD
! TUNE K SUCH THAT SIGMA_1 = SIGMA_2
! Scanning different dk to get the cloest result
! 
...
!=======================================================================
    ...
    REAL(P8),DIMENSION(:),ALLOCATABLE:: K_tune,K_store
...
105  FORMAT('         -(', I3,' - INPUT) (INPUT  >', I3, ')')
    ...
    K_VAL0 = 0.01 ! A random starting guess
    ALLOCATE(K_tune(1000))
    ALLOCATE(K_store(SIZE(K_tune)))

    DO JJ = 1,size(K_tune)
    ! Find how far away
        K_VAL1 = K_VAL0 + JJ*0.2
        ZLEN = 2*PI/K_VAL1
        CALL INIT_LOOP()
    ...
    CALL MSAVE(CMPLX(K_store,K_tune),'K_tunning.dat')   
