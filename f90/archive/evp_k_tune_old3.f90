...
105  FORMAT('         -(', I3,' - INPUT) (INPUT  >', I3, ')')
    ...
    ! Match the IMAG part
        tol_diff = MINVAL(ABS(AIMAG(M_eig0)-AIMAG(EIG_TARGET)))
        ...
        IF (tol_diff.LE.tol) THEN
            ...
            GOTO 20
        ELSE
            K_VAL0 = K_VAL0- K_step*(MINVAL(ABS(AIMAG(M_eig1)-AIMAG(EIG_TARGET))) - MINVAL(ABS(AIMAG(M_eig0)-AIMAG(EIG_TARGET))))/ABS(AIMAG(EIG_TARGET))/K_delta
