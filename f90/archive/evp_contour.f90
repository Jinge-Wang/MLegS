...
!=======================================================================
    ...
    INTEGER     :: M_IND,K_num,q_num
    REAL(P8)    :: K_VAL
    REAL(P8),DIMENSION(:),ALLOCATABLE:: K_range, q_range
    COMPLEX(P8),DIMENSION(:),ALLOCATABLE:: M_eig
    COMPLEX(P8),DIMENSION(:,:),ALLOCATABLE:: M_mat      !, EIG_R_mat, EIG_L_mat
    REAL(P8),DIMENSION(:,:),ALLOCATABLE:: Max_eig
...
105  FORMAT('         -(', I3,' - INPUT) (INPUT  >', I3, ')')
    ...
    ALLOCATE(K_range(K_num))
    ALLOCATE(q_range(q_num))
    ALLOCATE(Max_eig(K_num,q_num))
    K_range = (/ (II*(2.D0/(K_num-1)), II = 0, K_num-1) /)
    q_range = (/ (0.15+II*(-1.5-0.15)/(q_num-1), II = 0, q_num-1) /)

    DO II = 1,K_num
        K_VAL = K_range(II)
        DO JJ = 1,q_num
            QPAIR%Q = q_range(JJ)
            IF (K_VAL .EQ. 0.D0) THEN
                ZLEN = 251.32741228718345907701147066236
                CALL INIT_LOOP()
                CALL EIG_MATRIX(M_IND,1,K_VAL,M_mat,M_eig)
            ELSE
                ZLEN = 2*PI/K_VAL
                CALL INIT_LOOP()
                CALL EIG_MATRIX(M_IND,2,K_VAL,M_mat,M_eig)
            ENDIF
            Max_eig(II,JJ) = maxval(real(M_eig))
            WRITE(*,*) cmplx(II,JJ)
            ! WRITE(*,*) K_VAL
            ! WRITE(*,*) maxval(real(M_eig))
        ENDDO
    ENDDO

    CALL MSAVE(K_range,'K_range.dat')
    CALL MSAVE(q_range,'q_range.dat')
    CALL MSAVE(Max_eig,'Max_eig.dat')

    DEALLOCATE(K_range,q_range,Max_eig)
...
!=======================================================================
FUNCTION EIG_EVAL(KVAL,MIND,SIG)
    IMPLICIT NONE
    INTEGER:: MIND
    REAL(P8):: KVAL,EIG_EVAL
    REAL(P8),DIMENSION(:),ALLOCATABLE:: EIG_EVAL_LIST
    COMPLEX(P8):: SIG
    COMPLEX(P8),DIMENSION(:),ALLOCATABLE:: Meig
    COMPLEX(P8),DIMENSION(:,:),ALLOCATABLE:: Mmat

    ZLEN = 2*PI/KVAL
    CALL INIT_LOOP()
    CALL EIG_MATRIX(MIND,2,KVAL,Mmat,Meig)
    WRITE(*,*) ''
    WRITE(*,*) KVAL
    ALLOCATE(EIG_EVAL_LIST(SIZE(Meig)))
    EIG_EVAL_LIST = AIMAG(Meig-SIG)
    EIG_EVAL = EIG_EVAL_LIST(MINLOC(ABS(EIG_EVAL_LIST),1)) ! CLOSEST DISTANCE
    WRITE(*,*) EIG_EVAL

    DEALLOCATE(Meig,Mmat,EIG_EVAL_LIST)
END FUNCTION EIG_EVAL
!=======================================================================
SUBROUTINE ROOT_RANGE(LB,UB,UMAX,STEP,MIND,SIG)
    IMPLICIT NONE
    REAL(P8),INTENT(INOUT):: LB,UB
    
    INTEGER:: II,MIND
    COMPLEX(P8):: SIG
    REAL(P8):: STEP,LMIN,UMAX,LB_DIST,UB_DIST

    LMIN = 4.0
    LB_DIST = EIG_EVAL(LMIN,MIND,SIG)

    DO II = 1, CEILING((UMAX-LMIN)/STEP)
        UB = LMIN + STEP*II
        LB = LMIN + STEP*(II-1)
        UB_DIST = EIG_EVAL(UB,MIND,SIG)
...
! 210 DO II = 1, FLOOR((UB-LMIN)/STEP)
!         IF (UB - STEP*II .LE. LMIN) THEN
!             STEP = STEP*0.5
!             GOTO 210
!         ELSE
!             LB = UB - STEP*II
...
!         IF (LB.LE.LMIN) THEN
!             LB = LMIN
!             WRITE(*,*) 'ROOT_RANGE: LB = LMIN'
!             GOTO 211
!         ELSEIF (EIG_EVAL(LB,MIND,SIG)*UB_DIST.LT.0.D0) THEN
!             GOTO 211            
!         ENDIF
!     ENDDO
