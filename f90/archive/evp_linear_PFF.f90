...
! Having a test function in PFF space violates the BCs, and the mapping 
! to mapped Legendre space will not work!
...
PROGRAM EVP_LINEAR_PFF
    ...
    !=======================================================================
          ...
          TYPE(SCALAR):: PSI1,CHI1,PSI2,CHI2,PSI3,CHI3!,PSIW,CHIW
     ...
     106  FORMAT('M = ',I8,' & K = ',F8.3)
          ...
          ALLOCATE(EIG_VEC_3D(NR,2*NR,3))
    
          CALL ALLOCATE(PSIU,PFF_SPACE)
          CALL ALLOCATE(CHIU,PFF_SPACE)
          CALL ALLOCATE(RURU)
          CALL ALLOCATE(RUPU)
          CALL ALLOCATE( UZU)
          CALL ALLOCATE(RORU)
    ...
    ! PSI part
            ...
            CHI1%E = -CHI1%E + CHI2%E
            CALL IDEL2(CHI1,CHI1)
            CHI1%E = CHI1%E + CHI3%E
            CALL RTRAN(PSI1,1)
            CALL RTRAN(CHI1,1)
    
            H(:NR,  I) = PSI1%E(:NR,MIND,AKIND)
            H(NR+1:,I) = CHI1%E(:NR,MIND,AKIND) 
    ...
    ! CHI part
            ...
            H(:NR,  I+NR) &
                                = PSI1%E(:NR,MIND,AKIND)
            H(NR+1:,I+NR) &
                                = CHI1%E(:NR,MIND,AKIND) 
     ...
     107  FORMAT('PROGRESS: ',I3.3,'/',I3.3)
          ...
          CALL MSAVE(GENEIG(H), TRIM(ADJUSTL(FILES%SAVEDIR))// 'eigvalPFF_m_'// ITOA4(MREAD) //&
              '_k_'       // TRIM(ADJUSTL(STR1))  //&
              '_L_'       // TRIM(ADJUSTL(STR2))  //&
              '_ReI_'     // TRIM(ADJUSTL(STR3))  //&
              '_QI_'      // TRIM(ADJUSTL(STR4))  //&
        ...
        !   ! CREATE EIG_VEC_3D Matrix in PFF space:
        !   H = EIGVEC('R',H)
        !   EIG_VEC_3D = 0.D0
    ...
    1      DEALLOCATE( H )
          ...
          ! CALL DEALLOCATE(PSIW)
          ! CALL DEALLOCATE(CHIW)
