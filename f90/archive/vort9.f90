...
do it=1,iii
   ...
   eig0 = LOG(EMK(MONITOR_MK(1,1)+1,MONITOR_MK(1,2)+1)/EMK0(MONITOR_MK(1,1)+1,MONITOR_MK(1,2)+1))/(2*TIM%DT)
   ...
   IF (MOD(it,100).EQ.0) THEN
      ...
      IF (MPI_RANK.EQ.0) THEN   
         ...
         WRITE(*,*) 'STEP: ',it,'/',iii
         WRITE(*,*) '{M0,K0}: ',eig0,' - ',EMK(MONITOR_MK(1,1)+1,MONITOR_MK(1,2)+1)
...
IF (MPI_RANK.EQ.0) THEN
   ...
   CALL SAVE_PERTURB('./converg/new_perturb_final.input', &
      ...
      M(MONITOR_MK(1,1)), AK(MONITOR_MK(1,1),MONITOR_MK(1,2)),vec_bar, CMPLX(eig0))
