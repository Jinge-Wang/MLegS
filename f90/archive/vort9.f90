...
DO II = 1,SIZE(PSI%E,2)
   DO JJ = 1,SIZE(PSI%E,3)
      IF (((II+PSI%INTH .EQ. MONITORDATA%MK(1,1)+1).AND.(JJ+PSI%INX .EQ. MONITORDATA%MK(1,2)+1)) &
         .OR. ((II+PSI%INTH .EQ. MONITORDATA%MK(2,1)+1).AND.(JJ+PSI%INX .EQ. MONITORDATA%MK(2,2)+1)) &
...
converg_flag1 = 0
converg_flag2 = 0
eig00 = 0.d0; eig10 = 0.d0; eig20 = 0.d0
...
do it=1,iii
   ...
   eig0 = LOG(EMK(MONITORDATA%MK(1,1)+1,MONITORDATA%MK(1,2)+1)/EMK0(MONITORDATA%MK(1,1)+1,MONITORDATA%MK(1,2)+1))/(2*TIM%DT)
   ...
   IF (MOD(it,100).EQ.0) THEN
      ...
      IF (MPI_RANK.EQ.0) THEN   
         ...
         WRITE(*,*) 'STEP: ',it,'/',iii
         WRITE(*,*) '{M0,K0}: ',eig0,' - ',EMK(MONITORDATA%MK(1,1)+1,MONITORDATA%MK(1,2)+1)
         ...
         WRITE(666,134) tim%t,eig0,eig1,eig2
         close(666)
   ...
   eig00 = eig0; eig10 = eig1; eig20 = eig2
...
IF (MPI_RANK.EQ.0) THEN
   ...
   CALL SAVE_PERTURB('./converg/new_perturb_final.input', &
      ...
      M(MONITORDATA%MK(1,1)), AK(MONITORDATA%MK(1,1),MONITORDATA%MK(1,2)),vec_bar, CMPLX(eig0))
...
! ======================================================================
   ...
   DO I1 = 1,SIZE(PSI1%E,2); DO J1 = 1,SIZE(PSI1%E,3)
      ! IF (((I1+PSI1%INTH .EQ. MONITORDATA%MK(2,1)+1).AND.(J1+PSI1%INX .EQ. MONITORDATA%MK(2,2)+1)) &
      !    .OR. ((I1+PSI1%INTH .EQ. MONITORDATA%MK(3,1)+1).AND.(J1+PSI1%INX .EQ. MONITORDATA%MK(3,2)+1))) THEN
      ...
      !    CYCLE
      ! ENDIF
      IF ((I1+PSI1%INTH .EQ. MONITORDATA%MK(2,1)+1).AND.(J1+PSI1%INX .EQ. MONITORDATA%MK(2,2)+1)) THEN
         ...
         CYCLE
      ENDIF
      IF ((I1+PSI1%INTH .EQ. MONITORDATA%MK(3,1)+1).AND.(J1+PSI1%INX .EQ. MONITORDATA%MK(3,2)+1)) THEN
...
257 FORMAT(F10.3,',',(S,E26.15E3,SP,E26.15E3,'i'))
      ! M = 0, K != 0
      IF (((MONITORDATA%MK(2,1).EQ.0).AND.(I1+PSI1%INTH .EQ. MONITORDATA%MK(2,1)+1).AND.(J1+PSI1%INX .EQ. 2*NXCHOP-MONITORDATA%MK(2,2))) &
         .OR. ((MONITORDATA%MK(3,1).EQ.0).AND.(I1+PSI1%INTH .EQ. MONITORDATA%MK(3,1)+1).AND.(J1+PSI1%INX .EQ. 2*NXCHOP-MONITORDATA%MK(3,2)))) CYCLE
...
! ======================================================================
   ...
   DO I1 = 1,SIZE(PSI1%E,2); DO J1 = 1,SIZE(PSI1%E,3)
      ...
      ! M = 0, K != 0
      IF (((MONITORDATA%MK(2,1).EQ.0).AND.(I1+PSI1%INTH .EQ. MONITORDATA%MK(2,1)+1).AND.(J1+PSI1%INX .EQ. 2*NXCHOP-MONITORDATA%MK(2,2))) &
         .OR. ((MONITORDATA%MK(3,1).EQ.0).AND.(I1+PSI1%INTH .EQ. MONITORDATA%MK(3,1)+1).AND.(J1+PSI1%INX .EQ. 2*NXCHOP-MONITORDATA%MK(3,2)))) THEN
