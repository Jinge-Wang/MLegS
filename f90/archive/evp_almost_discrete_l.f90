...
REAL(P8)    :: K_BAR, K_VAL_1_0, K_VAL_1_1, K_VAL_2_0, K_VAL_2_1, K_1
...
COMPLEX(P8), DIMENSION(:, :), ALLOCATABLE:: P_mat_1, P_mat_2
...
INTEGER:: FID
! DEBUG:
COMPLEX(P8), DIMENSION(:, :), ALLOCATABLE:: EIG_NORM_CHECK
...
CALL EIG_MATRIX(M_BAR, K_BAR, M_mat, M_eig, EIG_R_mat, EIG_L_mat, comm_grp=newcomm) !, switch=.true.) ! calculate operator matrix and its corresponding eigenvalues
...
DO II = 1,6
    ...
    IF (MPI_GLB_RANK.EQ.0) THEN
        ...
        tol_diff = MINVAL(ABS(AIMAG(MESHGRID(M_eig_1_1, M_eig_2_1) - EIG_BAR)),MESHGRID(EIGRES(EIG_R_mat_1_1,M_1),EIGRES(EIG_R_mat_2_1,M_2)))
...
! ! =============================== DEBUG ================================
! IF (MPI_GLB_RANK.EQ.0) THEN
!     ALLOCATE(EIG_NORM_CHECK(SIZE(EIG_R_mat_1_1,1),SIZE(EIG_R_mat_1_1,1)))
!     EIG_NORM_CHECK = TRANSPOSE(CONJG(EIG_L_mat_1_1))*EIG_R_mat_1_1
!     WRITE(*,*) (EIG_NORM_CHECK(II,II),SUM(EIG_NORM_CHECK(II,1:10)),II=1,10)
!     DEALLOCATE(EIG_NORM_CHECK)
! ENDIF
! ! =============================== DEBUG ================================
...
IF (MPI_GLB_RANK .EQ. 0) THEN
    ...
    call SAVE_VEL(ROR_1, ROP_1, OZ_1, './converg/vor_MK_'//ITOA3(M_1)//'_'//ITOA4(INT(K_VAL_1_1*100)) &
                //'_IND_'//ITOA3(JJ)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
    CALL TEST_ENSTROPHY(CMPLX(ROR0),CMPLX(ROP0),CMPLX(OZ0))
    CALL TEST_ENSTROPHY(ROR_BAR,ROP_BAR,OZ_BAR)
    CALL TEST_ENSTROPHY(ROR_1, ROP_1, OZ_1)
    ...
    call SAVE_VEL(ROR_2, ROP_2, OZ_2, './converg/vor_MK_'//ITOA3(M_2)//'_'//ITOA4(INT(K_VAL_2_1*100)) &
    ...
    CALL TEST_ENSTROPHY(CMPLX(ROR0),CMPLX(ROP0),CMPLX(OZ0))
    CALL TEST_ENSTROPHY(ROR_BARC,ROP_BARC,OZ_BARC)
    CALL TEST_ENSTROPHY(ROR_2, ROP_2, OZ_2)
    ! CALL MPI_ABORT(MPI_COMM_WORLD,1,IERR)
...
! CALL PERTURB_MATRIX(M_BAR,K_BAR,RUR_BAR,RUP_BAR,UZ_BAR,ROR_BAR,ROP_BAR,OZ_BAR,M_1,K_VAL_1_1,P_mat_2,comm_grp=newcomm,switch=.TRUE.)
! CALL PERTURB_MATRIX(-M_BAR,-K_BAR,RUR_BARC,RUP_BARC,UZ_BARC,ROR_BARC,ROP_BARC,OZ_BARC,M_2,K_VAL_2_1,P_mat_1,comm_grp=newcomm,switch=.TRUE.)
CALL PERTURB_MATRIX2(M_BAR,K_BAR,RUR_BAR,RUP_BAR,UZ_BAR,ROR_BAR,ROP_BAR,OZ_BAR,M_1,K_VAL_1_1,P_mat_2,comm_grp=newcomm,switch=.TRUE.)
CALL PERTURB_MATRIX2(-M_BAR,-K_BAR,RUR_BARC,RUP_BARC,UZ_BARC,ROR_BARC,ROP_BARC,OZ_BARC,M_2,K_VAL_2_1,P_mat_1,comm_grp=newcomm,switch=.TRUE.)
! CALL MPI_ABORT(MPI_COMM_WORLD,1,IERR)
...
LAMB_2(KK) = 999; LAMB_UP = MINVAL(LAMB_2); !CALL MCAT(LAMB_2)
DEALLOCATE (LAMB_2)
...
LAMB_1(JJ) = 999; LAMB_UP = MIN(LAMB_UP, MINVAL(LAMB_1)); !CALL MCAT(LAMB_1)
DEALLOCATE (LAMB_1)
...
! ============================ DIAGNOSTICS =============================
! WRITE(*,*) "A(+)BETA(-)L1VR2: ", ALPHA_L*(-BETA)*DOT_PRODUCT(VECL_1,VECR_B2)
! WRITE(*,*) "B(+)ALFA(-)L2VR1: ", BETA_L*(ALPHA)*DOT_PRODUCT(VECL_2,VECR_B1)
...
VECR_1_CORRECT = EIG_CORRECT_3(JJ, M_eig_1_1, EIG_R_mat_1_1, EIG_L_mat_1_1, VECR_B2, M_1, K_VAL_1_1, ALPHA, BETA, newcomm, .TRUE.)
VECR_2_CORRECT = EIG_CORRECT_3(KK, M_eig_2_1, EIG_R_mat_2_1, EIG_L_mat_2_1, VECR_B1, M_2, K_VAL_2_1, BETA, ALPHA, newcomm, .TRUE.)
...
VECR_1 = VECR_1*ALPHA
VECR_2 = VECR_2*BETA
VECL_1 = VECL_1*ALPHA_L
VECL_2 = VECL_2*BETA_L
...
!                 //'_IND_'//ITOA3(JJ)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
! WRITE(K_VAL,'(F05.2)') K_VAL_2_1
! call SAVE_VEL(RUR_2, RUP_2, UZ_2, './converg/vel_MK_'//ITOA3(M_2)//'_'//K_VAL &
!                 //'_IND_'//ITOA3(KK)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
WRITE(K_VAL,'(F05.2)') K_VAL_1_1              
call SAVE_VEL(RUR_1*ALPHA, RUP_1*ALPHA, UZ_1*ALPHA, './converg/vel_MK_'//ITOA3(M_1)//'_'//K_VAL &
                //'_IND_'//ITOA3(JJ)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
WRITE(K_VAL,'(F05.2)') K_VAL_2_1
call SAVE_VEL(RUR_2*BETA, RUP_2*BETA, UZ_2*BETA, './converg/vel_MK_'//ITOA3(M_2)//'_'//K_VAL &
                //'_IND_'//ITOA3(KK)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')

! 2. SAVE UR, UP, UZ OF THE CORRECTION TERMS
! VECR_1_CORRECT, VECR_2_CORRECT
CALL EIG2VEL(M_1, K_VAL_1_1, VECR_1_CORRECT, RUR_C_1, RUP_C_1, UZ_C_1, comm_grp=newcomm)
CALL EIG2VEL(M_2, K_VAL_2_1, VECR_2_CORRECT, RUR_C_2, RUP_C_2, UZ_C_2, comm_grp=newcomm)
WRITE(K_VAL,'(F05.2)') K_VAL_1_1 
call SAVE_VEL(RUR_C_1, RUP_C_1, UZ_C_1, './converg/COR_vel_MK_'//ITOA3(M_1)//'_'//K_VAL &
    //'_IND_'//ITOA3(JJ)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
CALL SAVE_PSICHI(VECR_1_CORRECT,'./converg/COR_pt_MK_'//ITOA3(M_1)//'_'//K_VAL &
        //'_IND_'//ITOA3(JJ)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
WRITE(K_VAL,'(F05.2)') K_VAL_2_1
call SAVE_VEL(RUR_C_2, RUP_C_2, UZ_C_2, './converg/COR_vel_MK_'//ITOA3(M_2)//'_'//K_VAL &
    //'_IND_'//ITOA3(KK)//'_NRCHOP_'//ITOA3(NRCHOP)//'.output')
...
! ======================================================================
    ...
    IF (MPI_GLB_RANK .EQ. 0) THEN  ! START OF THE SERIAL PART
        IF (PRESENT(switch)) THEN
            open (10, FILE='./converg/eigM_MK_'//ITOA3(M_END)//'_'//ITOA4(INT(AK_END*100)) &
                    //'.output', STATUS='unknown', ACTION='WRITE', IOSTAT=IS)
            if (IS .ne. 0) then
...
SUBROUTINE PERTURB_MATRIX2(M_0,AK_0,RUR_0,RUP_0,UZ_0,ROR_0,ROP_0,OZ_0,MREAD,AKREAD,H,comm_grp,switch)
! ======================================================================
! MREAD,AKREAD: mode interacted with perturbation
! M_0  ,  AK_0: mode of perturbation
! MREAD+M_0, AKREAD+AK_0: H
! ======================================================================
    ...
    ENDIF
    ! write(*,*) MAXVAL(ABS(RUR_PSI)), MAXVAL(ABS(RUP_PSI)), MAXVAL(ABS(UZ_PSI))
    ! write(*,*) MAXLOC(ABS(RUR_PSI)), MAXLOC(ABS(RUP_PSI)), MAXLOC(ABS(UZ_PSI))
    ! write(*,*) MAXVAL(ABS(RUR_CHI)), MAXVAL(ABS(RUP_CHI)), MAXVAL(ABS(UZ_CHI))
    ...
    IF (MPI_GLB_RANK .EQ. 0) THEN  ! START OF THE SERIAL PART
        IF (PRESENT(switch)) THEN
            ...
            END DO
            close (10)
            write (*, *) 'PERTURB MAT (M=',M_END,', AK=',AK_END,'):  max element = ', maxval(abs(H)),' - LOCATION:',maxloc(abs(H))
            write (*, *) 'PERTURB MAT: maxUR = ', maxval(abs(RUR_0(1:NR)/TFM%R))
            write (*, *) 'PERTURB MAT: maxUP = ', maxval(abs(RUP_0(1:NR)/TFM%R))
            write (*, *) 'PERTURB MAT: maxUZ = ', maxval(abs( UZ_0(1:NR)))
            write (*, *) 'PERTURB MAT: maxOR = ', maxval(abs(ROR_0(1:NR)/TFM%R))
...
END PROGRAM EVP_ALMOST_DISCRETE_L
!=======================================================================
